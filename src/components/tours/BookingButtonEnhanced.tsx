...
import { validateTourPrice } from '@/lib/pricing/priceValidator'
...
export default function BookingButtonEnhanced({ 
  tour, 
  context = 'tour-detail',
  variant = 'default',
  className, 
  ...
  const getButtonText = () => {
    ...
    const priceString = tour.price || (tour.priceMin ? `€${tour.priceMin}` : null)
    const priceValidation = validateTourPrice(priceString)
    
    if (!priceValidation.isValid) {
      // No valid price - use softer CTA
      return context === 'tour-card' 
        ? 'View Details & Price' 
        : 'Check Price & Availability'
    }
    
    // Valid price exists - use stronger CTA
    return enhancedTour?.callToAction || 'Book Now'
  }
  ...
  const handleContinueToPartner = () => {
    try {
      ...
      console.error('Error redirecting to partner:', error)
    }
  }
    ...
    return (
      <>
        ...
        <RedirectModal
          ...
          partnerName={tour.operator?.name || 'BNAdventure'}
